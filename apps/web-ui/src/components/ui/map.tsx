...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Helper: fetch a style URL and inject the 'projection' field required by MapLibre GL v5.
  // Carto-based styles (positron, dark-matter, etc.) don't include 'projection',
  // which causes migrateProjection() to crash during style load.
  const injectProjection = useCallback(
    async (styleUrl: MapStyleOption): Promise<MapLibreGL.StyleSpecification | string> => {
      if (typeof styleUrl !== 'string') return styleUrl; // Already a full style object
      try {
        const res = await fetch(styleUrl);
        if (!res.ok) return styleUrl; // Network error — fall back to URL and let MapLibre handle it
        const json = await res.json() as MapLibreGL.StyleSpecification;
        return { ...json, ...(!json.projection ? { projection: { type: 'mercator' as const } } : {}) };
      } catch {
        return styleUrl; // Fetch failed — fall back to URL
      }
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Pre-fetch and patch the style so migrateProjection doesn't crash
      const initialStyle = await injectProjection(styleUrl);
      ...
      const loadHandler = () => { if (mounted) setIsLoaded(true); };
      safetyTimeout = setTimeout(loadHandler, 2000); // Reveal map even if 'load' event is slow
      ...
      map.on("click", (e) => {
        onClick?.({ lngLat: { lng: e.lngLat.lng, lat: e.lngLat.lat } });
      });
      if (mounted) setMapInstance(map);
    };
  ...
  // Handle style change (theme toggle)
  useEffect(() => {
    ...
    // Pre-fetch and patch so projecton is present (same fix as init)
    injectProjection(newStyleUrl).then((newStyle) => {
      if (!mapInstance) return;
      // transformStyle IS valid in setStyle options (unlike MapOptions)
      mapInstance.setStyle(newStyle, { diff: true });
    });
  }, [mapInstance, resolvedTheme, mapStyles, clearStyleTimeout, injectProjection]);
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-border bg-white dark:bg-slate-900 [&>button:not(:last-child)]:border-border flex flex-col overflow-hidden rounded-md border shadow-lg [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 flex size-8 items-center justify-center transition-colors",
...
type MapSearchProps = {
  ...
  placeholder?: string;
  className?: string;
  /** Nominatim viewbox to restrict results: "lon_min,lat_min,lon_max,lat_max" */
  viewbox?: string;
  /** ISO 3166-1 alpha-2 country code(s) to restrict results, e.g. "in" */
  countrycodes?: string;
  /** If true, only fires the search when the user presses Enter or clicks the search icon (no auto-search on type) */
  searchOnSubmit?: boolean;
  onResultSelect?: (result: { longitude: number; latitude: number; name: string }) => void;
};
...
function MapSearch({
  ...
  }, [viewbox, countrycodes]);

  const handleInputChange = (val: string) => {
    setQuery(val);
    if (searchOnSubmit) return; // user must press Enter / click button
