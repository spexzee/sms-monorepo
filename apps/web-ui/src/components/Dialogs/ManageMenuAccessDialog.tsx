...
const ManageMenuAccessDialog: React.FC<ManageMenuAccessDialogProps> = ({
    ...
    const handleToggleRole = async (role: string) => {
        ...
        try {
            ...
            const isDeactivated = currentDeactivated.includes(role);
            const newDeactivated = isDeactivated
                ? currentDeactivated.filter((r) => r !== role)
                : [...currentDeactivated, role];
    ...
    const handleToggleSchool = async (schoolId: string) => {
        ...
        try {
            const currentDeactivated = menu.deactivatedSchools || [];
            const isDeactivated = currentDeactivated.includes(schoolId);
            const newDeactivated = isDeactivated
                ? currentDeactivated.filter((s) => s !== schoolId)
                : [...currentDeactivated, schoolId];
